...
#define ASSERT_CONTAINS(haystack, needle, label)                                                              \
	do {                                                                                                      \
		if (std::string (haystack).find (needle) != std::string::npos) {                                      \
			...
			FAIL (label, "'" + std::string (haystack) + "' does not contain '" + std::string (needle) + "'"); \
		}                                                                                                     \
	}                                                                                                         \
	while (0)
...
#define ASSERT_ELEMENT_EXISTS(session, selector)               \
	do {                                                       \
		try {                                                  \
			...
			FAIL ("Element " selector " exists", "not found"); \
		}                                                      \
	}                                                          \
	while (0)
...
void
navigate_and_wait_for_models (webdriver::Session& session) {
	session.navigate (base_url);
	session.wait_until (
		"return document.querySelectorAll('#model-select option').length > 0", 5000);
}

void
test_page_loads (webdriver::Session& session) {
...
void
test_model_dropdown (webdriver::Session& session) {
...
void
test_translation_sse (webdriver::Session& session) {
	...
	bool has_output = session.wait_until (
		"return document.querySelector('#output-text').textContent.length > 0", 120000);
	...
	bool busy_cleared =
		session.wait_until ("return !document.body.classList.contains('busy')", 120000);
	ASSERT_TRUE (busy_cleared, "Busy state cleared after translation");
}

void
test_busy_state (webdriver::Session& session) {
	...
	session.execute_script ("document.querySelector('#output-text').textContent = ''");

	input.send_keys ("I had a great time visiting Tokyo last spring. "
					 "The cherry blossoms were beautiful and the food was amazing.");

	// Check busy state (debounce 300ms then translation starts)
	bool went_busy =
		session.wait_until ("return document.body.classList.contains('busy')", 5000);
...
void
test_empty_input (webdriver::Session& session) {
...
void
test_target_lang_selector (webdriver::Session& session) {
...
void
test_model_switch (webdriver::Session& session) {
	...
	session.execute_script ("document.querySelector('#model-select').value = '" + other_value +
							"';"
							"document.querySelector('#model-select').dispatchEvent("
							"  new Event('change'));");

	// Wait for model switch to complete (SSE: downloading → loading → ready)
	bool ready = session.wait_until ("return !document.body.classList.contains('busy')", 120000);
	ASSERT_TRUE (ready, "Model switch completed");

	auto new_value = session.execute_script (
		"return document.querySelector('#model-select').value");
	ASSERT_TRUE (new_value == other_value, "Model changed to '" + other_value + "'");
}

void
test_download_dialog_structure (webdriver::Session& session) {
