...
namespace webdriver {
	...
	public:
		Element (httplib::Client* cli, const std::string& session_id, const std::string& element_id)
			: cli_ (cli), session_id_ (session_id), element_id_ (element_id) {}

		std::string
		url (const std::string& suffix = "") const {
	...
	public:
		...
		click () const {
			...
			if (!res || res->status != 200) {
				throw Error ("Failed to click element");
			}
		}

		void
		send_keys (const std::string& keys) const {
			json body = {{"text", keys}};
		...
		clear () const {
			...
			if (!res || res->status != 200) {
				throw Error ("Failed to clear element");
			}
		}
		...
		tag_name () const {
			...
			if (!res || res->status != 200) {
				throw Error ("Failed to get tag name");
			}
			return json::parse (res->body) ["value"].get<std::string> ();
		}
	...
	class Session {
		...
		std::string
		extract_element_id (const json& value) const {
	...
	public:
		...
		void
		navigate (const std::string& nav_url) {
		...
		title () {
			...
			if (!res || res->status != 200) {
				throw Error ("Failed to get title");
			}
			return json::parse (res->body) ["value"].get<std::string> ();
		}
		...
		Element
		find (const std::string& using_, const std::string& value) {
		...
		std::vector<Element>
		find_all (const std::string& using_, const std::string& value) {
			json body = {{"using", using_}, {"value", value}};
		...
		Element
		css (const std::string& selector) {
			return find ("css selector", selector);
		}

		std::vector<Element>
		css_all (const std::string& selector) {
		...
		Element
		wait_for (const std::string& selector, int timeout_ms = 5000) {
		...
		bool
		wait_until (const std::string& script, int timeout_ms = 5000) {
			auto deadline = std::chrono::steady_clock::now () + std::chrono::milliseconds (timeout_ms);
			while (std::chrono::steady_clock::now () < deadline) {
		...
		std::string
		execute_script (const std::string& script, const json& args = json::array ()) {
