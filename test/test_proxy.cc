...
std::string
normalizeJson (const std::string& json) {
	std::string result;
	for (char c: json) {
		if (c != ' ' && c != '\t' && c != '\n' && c != '\r') {
			result += c;
...
template <typename T>
void
RedirectProxyText (T& cli, const char* path, bool basic) {
	cli.set_proxy ("localhost", basic ? 3128 : 3129);
	if (basic) {
...
TEST (RedirectTest, TLSVerificationOnProxyRedirect) {
	...
	redirect_svr.Get ("/", [&] (const Request&, Response& res) {
		res.set_redirect ("https://host.docker.internal:" + to_string (untrusted_port) + "/");
	});
...
template <typename T>
void
BaseAuthTestFromHTTPWatch (T& cli) {
	cli.set_proxy ("localhost", 3128);
	cli.set_proxy_basic_auth ("hello", "world");
...
template <typename T>
void
DigestAuthTestFromHTTPWatch (T& cli) {
...

	{
		...
		for (auto path: paths) {
			...
			EXPECT_EQ (normalizeJson ("{\"algorithm\":\"" + algo + "\",\"authenticated\":true,\"user\":\"hello\"}\n"),
					   normalizeJson (res->body));
			EXPECT_EQ (StatusCode::OK_200, res->status);
		}
...
template <typename T>
void
KeepAliveTest (T& cli, bool basic) {
	cli.set_proxy ("localhost", basic ? 3128 : 3129);
	if (basic) {
		cli.set_proxy_basic_auth ("hello", "world");
	}
