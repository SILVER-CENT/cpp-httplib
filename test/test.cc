...
FormData&
get_file_value (std::vector<FormData>& items, const char* key) {
...
protected:
	...
	void
	client_GET (const std::string& addr) {
	...
	make_sock_path () {
		const char* shard = getenv ("GTEST_SHARD_INDEX");
		return shard ? std::string ("./httplib-server-") + shard + ".sock" : "./httplib-server.sock";
	}
...
#if defined(__linux__) || /* __APPLE__ */ (defined(SOL_LOCAL) && defined(SO_PEERPID))
...
TEST (SocketStream, wait_writable_UNIX) {
	...
	const auto asSocketStream = [&] (socket_t fd, std::function<bool (Stream&)> func) {
...
TEST (SetSocketOptTest, TcpNoDelay) {
	...
	ASSERT_EQ (0, ::getsockopt (sock, IPPROTO_TCP, TCP_NODELAY, reinterpret_cast<char*> (&val), &len));
	EXPECT_NE (val, 0);
...
TEST (DecodePathTest, PercentCharacter) {
	EXPECT_EQ (decode_path_component (R"(descrip=Gastos%20%C3%A1%C3%A9%C3%AD%C3%B3%C3%BA%C3%B1%C3%91%206)"),
...
TEST (SanitizeFilenameTest, VariousPatterns) {
	...
	EXPECT_EQ ("filename.txt", httplib::sanitize_filename ("/path/to/filename.txt"));
	EXPECT_EQ (".gitignore", httplib::sanitize_filename (".gitignore"));
	EXPECT_EQ ("", httplib::sanitize_filename (".."));
	EXPECT_EQ ("", httplib::sanitize_filename (""));
	// Null bytes stripped
	EXPECT_EQ ("safe.txt", httplib::sanitize_filename (std::string ("safe\0.txt", 9)));
...
TEST (EncodeQueryParamTest, ParseReservedCharactersTest) {
	...
	EXPECT_EQ (httplib::encode_uri_component (reservedCharacters), "%3B%2C%2F%3F%3A%40%26%3D%2B%24");
}
...
TEST (EncodeQueryParamTest, TestUTF8Characters) {
	...
	EXPECT_EQ (httplib::encode_uri_component (chineseCharacters), "%E4%B8%AD%E5%9B%BD%E8%AA%9E");

	EXPECT_EQ (httplib::encode_uri_component (russianCharacters), "%D0%B4%D0%BE%D0%BC");
...
TEST (EncodeUriTest, TestUTF8Characters) {
	...
	EXPECT_EQ (httplib::encode_uri (chineseCharacters), "%E4%B8%AD%E5%9B%BD%E8%AA%9E");
...
TEST (EncodeUriTest, TestCompleteUri) {
	string uri = "https://example.com/path/to/resource?query=value&param=test#fragment";
...
TEST (EncodeUriTest, TestUriWithSpacesAndSpecialChars) {
	string uri = "https://example.com/path with spaces/file name.html?q=hello world";
...
TEST (DecodeUriComponentTest, TestPathComponentDecoding) {
	...
	EXPECT_EQ (httplib::decode_uri_component (encodedPath), "Piri Tommy Villiers - on & on");
}
...
TEST (DecodeUriTest, TestCompleteUri) {
	...
	EXPECT_EQ (httplib::decode_uri (encodedUri),
			   "https://example.com/path with spaces/file name.html?q=hello world");
}

TEST (DecodeUriTest, TestRoundTripWithEncodeUri) {
	string original = "https://example.com/path with spaces/file name.html?q=hello world";
...
TEST (ParseAcceptHeaderTest, BasicAcceptParsing) {
	...
	EXPECT_TRUE (detail::parse_accept_header ("text/html,application/json,text/plain", result1));
...
TEST (ParseAcceptHeaderTest, MixedQualityValues) {
	...
	EXPECT_TRUE (
		detail::parse_accept_header ("text/html,application/json;q=0.5,text/plain;q=0.8", result));
...
TEST (ParseAcceptHeaderTest, EdgeCases) {
	...
	EXPECT_TRUE (detail::parse_accept_header ("text/*;q=0.5,*/*;q=0.1,application/json", wildcard_result));
...
TEST (ParseAcceptHeaderTest, RealWorldExamples) {
	...
	EXPECT_TRUE (detail::parse_accept_header (
		"application/json;q=0.9,application/xml;q=0.8,text/plain;q=0.1", api_result));
...
TEST (ParseAcceptHeaderTest, SpecialCases) {
	...
	EXPECT_TRUE (
		detail::parse_accept_header ("text/html;q=0.123,application/json;q=0.456", decimal_result));
	...
	EXPECT_TRUE (detail::parse_accept_header ("text/html;q=0,application/json;q=1", zero_q_result));
	...
	EXPECT_TRUE (detail::parse_accept_header (
		"text/html;q=0.9,application/json;q=0.8,text/plain;q=0.7", no_space_result));
...
TEST (ParseAcceptHeaderTest, InvalidCases) {
	...
	EXPECT_FALSE (detail::parse_accept_header ("text/html;q=1.5,application/json", result));

	// Invalid quality value (< 0.0)
	EXPECT_FALSE (detail::parse_accept_header ("text/html;q=-0.1,application/json", result));

	// Invalid quality value (not a number)
	EXPECT_FALSE (detail::parse_accept_header ("text/html;q=invalid,application/json", result));

	// Empty quality value
	EXPECT_FALSE (detail::parse_accept_header ("text/html;q=,application/json", result));

	// Invalid media type format (no slash and not wildcard)
	EXPECT_FALSE (detail::parse_accept_header ("invalidtype,application/json", result));
...
TEST (ParseAcceptHeaderTest, ContentTypesPopulatedAndInvalidHeaderHandling) {
	...
	{
		auto res = cli.Get ("/accept_bad_request", {{"Accept", "text/html;q=abc,application/json"}});
		ASSERT_TRUE (res);
...
TEST (DivideTest, DivideStringTests) {
	auto divide = [] (const std::string& str, char d) {
		...
		detail::divide (str, d, [&] (const char* lhs_data, std::size_t lhs_size, const char* rhs_data, std::size_t rhs_size) {
...
TEST (ParseMultipartBoundaryTest, ValueWithQuotesAndCharset) {
	string content_type = "multipart/mixed; boundary=\"cpp-httplib-multipart-data\"; "
						  "charset=UTF-8";
	string boundary;
	auto ret = detail::parse_multipart_boundary (content_type, boundary);
	EXPECT_TRUE (ret);
...
TEST (GetHeaderValueTest, RegularInvalidValueInt) {
	...
	auto val = detail::get_header_value_u64 (headers, "Content-Length", 0, 0, is_invalid_value);
	EXPECT_EQ (0ull, val);
...
TEST (ParseHeaderValueTest, Range) {
	...
	{
		Ranges ranges;
		auto ret = detail::parse_range_header ("bytes=1-10, 100-200, 300-400", ranges);
		EXPECT_TRUE (ret);
...
protected:
	...
	SetUp () {
		...
		svr_.Get ("/chunked", [this] (const httplib::Request&, httplib::Response& res) {
			res.set_chunked_content_provider ("image/jpeg", [this] (size_t offset, httplib::DataSink& sink) {
...
TEST (RangeTest, FromHTTPBin_Online) {
	...
	// acceptable per RFC 9110 §15.3.7, so we accept either status code.
	{
		...
		ASSERT_TRUE (res);
		EXPECT_EQ ("abcdefghijklmnopqrstuvwxyzabcdef", res->body);
		EXPECT_TRUE (res->status == StatusCode::OK_200 || res->status == StatusCode::PartialContent_206);
	}
...
#if defined(__linux__) && defined(__GLIBC__) && defined(CPPHTTPLIB_USE_NON_BLOCKING_GETADDRINFO)
...
namespace {
	bool
	should_run_issue_2431_tests () {
		const char* v = getenv ("CPPHTTPLIB_TEST_ISSUE_2431");
		return v && *v && std::string (v) != "0";
	}

	std::string
	unique_unresolvable_host (int n) {
...
TEST (GetAddrInfoAsyncCancelTest, DirectCallSingleThread) {
	if (!should_run_issue_2431_tests ()) {
		GTEST_SKIP () << "Set CPPHTTPLIB_TEST_ISSUE_2431=1 (and sinkhole DNS) to run";
	}
	...
	for (int i = 0; i < 8; ++i) {
		...
		int rc = detail::getaddrinfo_with_timeout (host.c_str (), "80", &hints, &result, /*timeout_sec=*/1);
		if (rc == 0 && result) {
			freeaddrinfo (result);
		}
	}
...
TEST (ConnectionErrorTest, InvalidPort) {
	...
	auto res = cli.Get ("/");
	ASSERT_TRUE (!res);
	EXPECT_TRUE (Error::Connection == res.error () || Error::ConnectionTimeout == res.error ());
}
...
TEST (CancelTest, WithCancelLargePayload_Online) {
	...
	uint32_t count = 0;
	auto res = cli.Get (path, [&count] (uint64_t, uint64_t) { return (count++ == 0); });
	ASSERT_TRUE (!res);
	EXPECT_EQ (Error::Canceled, res.error ());
}
...
static std::string
remove_whitespace (const std::string& input) {
	std::string output;
...
TEST (BaseAuthTest, FromHTTPWatch_Online) {
	...
	{
		auto res = cli.Get (path, {make_basic_authentication_header ("hello", "world")});
		ASSERT_TRUE (res);
...
TestDoNotForwardCredentialsOnRedirect (std::function<void (Client&)> set_auth) {
	...
	svr2.Get ("/redir", [&] (const Request& /*req*/, Response& res) {
		res.set_redirect ("http://localhost:" + std::to_string (svr1_port) + "/target", 302);
	});
...
TEST (BindServerTest, BindAndListenSeparatelySSLEncryptedKey) {
	SSLServer svr (SERVER_ENCRYPTED_CERT_FILE, SERVER_ENCRYPTED_PRIVATE_KEY_FILE, nullptr, nullptr, SERVER_ENCRYPTED_PRIVATE_KEY_PASS);
	int port = svr.bind_to_any_port ("0.0.0.0");
	ASSERT_TRUE (svr.is_valid ());
	ASSERT_TRUE (port > 0);
	svr.stop ();
...
TEST (BindServerTest, UpdateCertsPem) {
	...
	ASSERT_TRUE (svr.update_certs_pem (cert_pem.c_str (), key_pem.c_str (), ca_pem.c_str ()));
...
TEST (ExceptionTest, AndErrorHandler) {
	...
	svr.set_error_handler ([] (const Request& /*req*/, Response& res) {
		if (res.body.empty ()) {
			res.set_content ("NOT_FOUND", "text/html");
		}
	});
...
TEST (RequestHandlerTest, PreRequestHandler) {
	...
	svr.Get ("/hi",
			 [] (const Request&, Response& res) { res.set_content ("hi", "text/plain"); });
...
protected:
        ...
        .Post("/slowpost",
			...
			.Get ("/local_addr",
				  [&] (const Request& req, Response& res) {
					  ...
					  res.set_content (local_addr.append (":").append (local_port), "text/plain");
				  })
			.Get ("/endwith%",
				  [&] (const Request& /*req*/, Response& res) {
					  res.set_content ("Hello World!", "text/plain");
					  ...
					  res.set_chunked_content_provider (
						  ...
						  [i] (size_t /*offset*/, DataSink& sink) {
							  switch (*i) {
								  ...
								  case 2: sink.os << "789"; break;
								  case 3: {
									  sink.done_with_trailer ({{"Dummy1", "DummyVal1"},
															   {"Dummy2", "DummyVal2"}});
								  } break;
							  }
							  (*i)++;
			...
			.Get ("/streamed-cancel",
				  [&] (const Request& /*req*/, Response& res) {
					  res.set_content_provider (size_t (-1),
												"text/plain",
												[] (size_t /*offset*/, size_t /*length*/, DataSink& sink) {
													sink.os << "data_chunk";
													return true;
			...
			.Post ("/multipart",
				   [&] (const Request& req, Response& /*res*/) {
					   EXPECT_EQ (4u,
								  req.form.get_field_count ("text1") +
									  req.form.get_field_count ("text2") +
									  req.form.get_field_count ("file3") +
									  req.form.get_field_count ("file4"));
					   EXPECT_EQ (2u, req.form.get_file_count ("file1") + req.form.get_file_count ("file2"));
			...
			.Post ("/multipart/multi_file_values",
				   [&] (const Request& req, Response& /*res*/) {
					   EXPECT_EQ (3u, req.form.get_field_count ("text") + req.form.get_field_count ("multi_text1"));
			...
			.Post ("/post-loopback",
				   [&] (const Request&, Response& res, ContentReader const& content_reader) {
					   std::string body;
			...
			.Post ("/binary",
				   [&] (const Request& req, Response& res) {
					   ...
					   EXPECT_EQ ("application/octet-stream", req.get_header_value ("Content-Type"));
	...
	defined(CPPHTTPLIB_ZSTD_SUPPORT)
			.Get ("/compress",
				  [&] (const Request& /*req*/, Response& res) {
					  res.set_content ("1234567890123456789012345678901234567890123456789"
									   "0123456789"
...
TEST_F (ServerTest, StaticFileRanges) {
	auto res = cli_.Get ("/dir/test.abcde", {{make_range_header ({{1, 2}, {4, -1}})}});
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::PartialContent_206, res->status);
	EXPECT_TRUE (res->get_header_value ("Content-Type").find ("multipart/byteranges; boundary=--cpp-httplib-multipart-data-") ==
				 0);
...
TEST_F (ServerTest, Binary) {
	...
	auto res = cli_.Post ("/binary", binary.data (), binary.size (), "application/octet-stream");
	ASSERT_TRUE (res);
	ASSERT_EQ (StatusCode::OK_200, res->status);
	ASSERT_EQ (4U, res->body.size ());

	res = cli_.Put ("/binary", binary.data (), binary.size (), "application/octet-stream");
	ASSERT_TRUE (res);
	ASSERT_EQ (StatusCode::OK_200, res->status);
	ASSERT_EQ (4U, res->body.size ());

	res = cli_.Patch ("/binary", binary.data (), binary.size (), "application/octet-stream");
	ASSERT_TRUE (res);
	ASSERT_EQ (StatusCode::OK_200, res->status);
	ASSERT_EQ (4U, res->body.size ());

	res = cli_.Delete ("/binary", binary.data (), binary.size (), "application/octet-stream");
	ASSERT_TRUE (res);
	ASSERT_EQ (StatusCode::OK_200, res->status);
	ASSERT_EQ (4U, res->body.size ());
}
...
TEST_F (ServerTest, MultipartFormData) {
	UploadFormDataItems items = {{"text1", "text default", "", ""},
								 {"text2", "aωb", "", ""},
...
TEST_F (ServerTest, CaseInsensitiveTransferEncoding) {
	...
	req.headers.emplace ("Transfer-Encoding",
...
static void
expect_chunked_body_rejected (ClientT& cli, const char* body) {
	Request req;
	req.method = "POST";
	req.path = "/chunked";
...
TEST_F (ServerTest, RejectsChunkSizeWithLeadingPlus) {
	expect_chunked_body_rejected (cli_, "+4\r\ndech\r\nf\r\nunked post body\r\n0\r\n\r\n");
}
...
TEST_F (ServerTest, GetStreamedWithRangeError) {
	auto res = cli_.Get ("/streamed-with-range",
						 {{"Range",
						   "bytes=92233720368547758079223372036854775806-"
...
TEST_F (ServerTest, GetStreamedWithRangeMultipart) {
	auto res = cli_.Get ("/streamed-with-range", {{make_range_header ({{1, 2}, {4, 5}})}});
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::PartialContent_206, res->status);
...
TEST_F (ServerTest, GetStreamedWithTooManyRanges) {
	...
	auto res = cli_.Get ("/streamed-with-range?error", {{make_range_header (ranges)}});
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::RangeNotSatisfiable_416, res->status);
	EXPECT_EQ ("0", res->get_header_value ("Content-Length"));
	EXPECT_EQ (false, res->has_header ("Content-Range"));
...
TEST_F (ServerTest, GetStreamedWithOverwrapping) {
	auto res = cli_.Get ("/streamed-with-range", {{make_range_header ({{1, 4}, {2, 5}})}});
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::PartialContent_206, res->status);
...
TEST_F (ServerTest, GetStreamedWithNonAscendingRanges) {
	auto res = cli_.Get ("/streamed-with-range", {{make_range_header ({{4, 5}, {0, 2}})}});
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::PartialContent_206, res->status);
...
TEST_F (ServerTest, GetStreamedWithDuplicateRanges) {
	auto res = cli_.Get ("/streamed-with-range", {{make_range_header ({{0, 2}, {0, 2}})}});
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::PartialContent_206, res->status);
...
TEST_F (ServerTest, GetStreamedEndless) {
	uint64_t offset = 0;
	auto res = cli_.Get ("/streamed-cancel", [&] (const char* /*data*/, uint64_t data_length) {
		if (offset < 100) {
			offset += data_length;
			return true;
		}
...
TEST_F (ServerTest, PostWithContentProviderAbort) {
	auto res = cli_.Post (
		"/post",
		42,
		[] (size_t /*offset*/, size_t /*length*/, DataSink& /*sink*/) { return false; },
		"text/plain");
...
TEST_F (ServerTest, PostWithContentProviderWithoutLengthAbort) {
	auto res = cli_.Post (
		"/post", [] (size_t /*offset*/, DataSink& /*sink*/) { return false; }, "text/plain");
...
TEST (GzipDecompressor, ChunkedDecompression) {
	...
	std::string compressed_data;
	{
		...
		bool result = compressor.compress (data.data (),
										   data.size (),
										   /*last=*/true,
										   [&] (const char* compressed_data_chunk, size_t compressed_data_size) {
											   compressed_data.insert (compressed_data.size (),
...
TEST_F (ServerTest, ArrayParamValues) {
	auto res = cli_.Get ("/array-param-values?array=value1&array=value2&array=value3");
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::OK_200, res->status);
}
...
void
TestWithHeadersAndContentReceiver (
	ClientType& cli,
	std::function<Result (ClientType&, const std::string&, const Headers&, const std::string&, const std::string&, ContentReceiver, DownloadProgress)> request_func) {
	Headers headers;
...
template <typename ClientType>
void
TestWithHeadersAndContentReceiverWithProgress (
	ClientType& cli,
	std::function<Result (ClientType&, const std::string&, const Headers&, const std::string&, const std::string&, ContentReceiver, DownloadProgress)> request_func) {
	Headers headers;
...
template <typename ClientType>
void
TestWithHeadersAndContentReceiverError (
	ClientType& cli,
	std::function<Result (ClientType&, const std::string&, const Headers&, const std::string&, const std::string&, ContentReceiver)> request_func) {
	Headers headers;
...
TEST_F (ServerTest, KeepAlive) {
	...
	res = cli_.Post ("/empty", 0, [&] (size_t, size_t, DataSink&) { return true; }, "text/plain");
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::OK_200, res->status);
	EXPECT_EQ ("text/plain", res->get_header_value ("Content-Type"));
	EXPECT_EQ ("empty", res->body);
...
TEST_F (ServerTest, GzipWithContentTypeParameters) {
	...
	ASSERT_TRUE (res);
	EXPECT_EQ ("gzip", res->get_header_value ("Content-Encoding"));
	EXPECT_EQ ("application/json; charset=utf-8", res->get_header_value ("Content-Type"));
	EXPECT_EQ ("123456789012345678901234567890123456789012345678901234567890123456"
			   "7890123456789012345678901234567890",
			   res->body);
	EXPECT_EQ (StatusCode::OK_200, res->status);
...
TEST_F (ServerTest, GzipWithContentReceiver) {
	...
	headers.emplace ("Accept-Encoding", "gzip, deflate");
	std::string body;
	auto res = cli_.Get ("/compress", headers, [&] (const char* data, uint64_t data_length) {
...
TEST_F (ServerTest, NoGzipWithContentReceiver) {
	...
	headers.emplace ("Accept-Encoding", "gzip, deflate");
	std::string body;
	auto res = cli_.Get ("/nocompress", headers, [&] (const char* data, uint64_t data_length) {
		EXPECT_EQ (100U, data_length);
		body.append (data, data_length);
		return true;
	});
...
TEST_F (ServerTest, ZstdWithoutDecompressing) {
	...
	unsigned char compressed [26] = {0x28, 0xb5, 0x2f, 0xfd, 0x20, 0x64, 0x8d, 0x00, 0x00,
									 0x50, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38,
									 0x39, 0x30, 0x01, 0x00, 0xd7, 0xa9, 0x20, 0x01};
...
TEST_F (ServerTest, PreCompressionLogging) {
	...
	const std::string test_content = "123456789012345678901234567890123456789012345678901"
...
TEST (ZstdDecompressor, Decompress) {
	...
	unsigned char data [29] = {0x28, 0xb5, 0x2f, 0xfd, 0x20, 0x14, 0xa1, 0x00, 0x00, 0x43,
							   0x6f, 0x6d, 0x70, 0x72, 0x65, 0x73, 0x73, 0x65, 0x64, 0x20,
							   0x77, 0x69, 0x74, 0x68, 0x20, 0x5a, 0x53, 0x54, 0x44};
	std::string compressed_data (data, data + sizeof (data) / sizeof (data [0]));
...
static bool
send_request (time_t read_timeout_sec, const std::string& req, std::string* resp = nullptr) {
	auto error = Error::Success;

	auto client_sock = detail::create_client_socket (HOST,
													 "",
													 PORT,
													 AF_UNSPEC,
													 false,
	...
	auto ret = detail::process_client_socket (
		client_sock, read_timeout_sec, 0, 0, 0, 0, std::chrono::steady_clock::time_point::min (), [&] (Stream& strm) {
			if (req.size () != static_cast<size_t> (strm.write (req.data (), req.size ()))) {
				return false;
			}
...
static void
test_raw_request (const std::string& req, std::string* out = nullptr) {
	Server svr;
	svr.Get ("/hi", [&] (const Request& /*req*/, Response& res) {
		res.set_content ("ok", "text/plain");
	});
...
TEST (ServerRequestParsingTest, InvalidFieldNameContains_PreceedingSpaces) {
	std::string out;
	std::string request ("GET /header_field_value_check HTTP/1.1\r\n  Test: val\r\n\r\n", 55);
	test_raw_request (request, &out);
	EXPECT_EQ ("HTTP/1.1 400 Bad Request", out.substr (0, 24));
}
...
TEST (ServerStopTest, StopServerWithChunkedTransmission) {
	...
	auto get_thread = std::thread ([&client, &headers] () {
		auto res = client.Get ("/events", headers, [] (const char* /*data*/, size_t /*len*/) -> bool {
			return true;
		});
	});
	auto se = detail::scope_exit ([&] {
...
void
TestStringBodyUploadProgress (SetupHandler&& setup_handler, ClientCall&& client_call, const string& body) {
	Server svr;
	setup_handler (svr);
	...
	auto res = client_call (cli, body, [&] (uint64_t current, uint64_t /*total*/) -> bool {
...
template <typename SetupHandler, typename ClientCall>
void
TestContentProviderUploadProgress (SetupHandler&& setup_handler, ClientCall&& client_call) {
	Server svr;
	setup_handler (svr);
	...
	auto res = client_call (cli, [&] (uint64_t current, uint64_t /*total*/) -> bool {
		progress_values.push_back (current);
		return true;
	});
...
template <typename SetupHandler, typename ClientCall>
void
TestMultipartUploadProgress (SetupHandler&& setup_handler, ClientCall&& client_call, const string& endpoint) {
	Server svr;
	setup_handler (svr);
	...
	UploadFormDataItems items = {{"field1", "value1", "", ""},
								 {"field2", "longer value for progress tracking test", "", ""},
								 {"file1", "file content data for upload progress", "test.txt", "text/plain"}};

	auto res = client_call (cli, endpoint, items, [&] (uint64_t current, uint64_t /*total*/) -> bool {
		progress_values.push_back (current);
		return true;
	});
...
TEST (UploadProgressTest, PostMultipartProgress) {
	TestMultipartUploadProgress (
		...
		[] (Client& cli, const string& endpoint, const UploadFormDataItems& items, UploadProgress progress_callback) {
...
template <typename SetupHandler, typename ClientCall>
void
TestBasicDownloadProgress (SetupHandler&& setup_handler,
	...
	auto res = client_call (cli, endpoint, [&] (uint64_t current, uint64_t /*total*/) -> bool {
		progress_values.push_back (current);
		return true;
	});
...
TEST (DownloadProgressTest, GetBasic) {
	TestBasicDownloadProgress (
		...
		[] (Client& cli, const string& endpoint, DownloadProgress progress_callback) {
...
template <typename SetupHandler, typename ClientCall>
void
TestContentReceiverDownloadProgress (SetupHandler&& setup_handler,
									 ClientCall&& client_call,
									 const string& endpoint,
									 size_t expected_content_size) {
	Server svr;
...
TEST (DownloadProgressTest, GetWithContentReceiver) {
	TestContentReceiverDownloadProgress (
		[] (Server& svr) {
			svr.Get ("/download-receiver", [] (const Request& /*req*/, Response& res) {
				string content (2000, 'R');
				res.set_content (content, "text/plain");
			});
		},
		[] (Client& cli, const string& endpoint, ContentReceiver content_receiver, DownloadProgress progress_callback) {
...
TEST (MmapTest, OpenWhileFileHeldForWriting) {
	...
	HANDLE writer = ::CreateFileA (path, GENERIC_WRITE, FILE_SHARE_READ, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, NULL);
...
TEST (ClientProblemDetectionTest, ContentProvider) {
	...
	{
		auto res = cli.Get ("/empty", [&] (const char* /*data*/, size_t /*data_length*/) {
			return false;
		});
		ASSERT_TRUE (res);
	}
...
TEST (ErrorHandlerWithContentProviderTest, ErrorHandler) {
	...
	svr.set_error_handler ([] (Request const&, Response& res) -> void {
		res.set_chunked_content_provider ("text/plain", [] (std::size_t const, DataSink& sink) -> bool {
			sink.os << "hello";
			sink.os << "world";
			sink.done ();
			return true;
...
TEST_F (PayloadMaxLengthTest, RawSocketChunkedTest) {
	...
	if (!result) {
		...
		SUCCEED () << "Server rejected oversized chunked request (connection closed)";
	}
	else {
...
TEST_F (PayloadMaxLengthTest, NoContentLengthWithinLimit) {
	...
	if (result) {
		...
		if (response.length () > 10) {
			SUCCEED () << "Server processed request without Content-Length within limit";
		}
		else {
...
TEST_F (LargePayloadMaxLengthTest, NoContentLengthExceeds10MB) {
	...
	if (!result) {
		...
		if (response.length () <= 10) {
			SUCCEED () << "Server closed connection for 12MB request exceeding 10MB "
						  "limit";
		}
		else {
			EXPECT_TRUE (response.find ("413") != std::string::npos ||
...
TEST (PayloadLimitBypassTest, StreamingGzipDecompression) {
	...
	size_t total = 0;
	svr.Post ("/stream", [&] (const Request& /*req*/, Response& res, const ContentReader& content_reader) {
	...
	auto res = cli.Post ("/stream", headers, gz.data (), gz.size (), "application/octet-stream");
	ASSERT_TRUE (res);
...
TEST (ClientVulnerabilityTest, ContentReceiverBypassesDefaultPayloadMaxLength) {
...

	{
		...
		auto res = cli.Get ("/large", [&] (const char* /*data*/, size_t data_length) {
...
TEST (ClientVulnerabilityTest, ContentReceiverRespectsExplicitPayloadMaxLength150MB) {
	static constexpr size_t DATA_SIZE = 200 * 1024 * 1024;	// 200MB from server
...
TEST (ClientVulnerabilityTest, ContentReceiverRespectsExplicitPayloadMaxLength250MB) {
	static constexpr size_t DATA_SIZE = 200 * 1024 * 1024;	// 200MB from server
...
TEST (ClientVulnerabilityTest, ZipBombWithoutContentLength) {
	constexpr size_t DECOMPRESSED_SIZE = 10 * 1024 * 1024;	// 10MB after decompression
	constexpr size_t CLIENT_READ_LIMIT = 2 * 1024 * 1024;  // 2MB safety limit
	...
	{
		httplib::detail::gzip_compressor compressor;
		bool ok = compressor.compress (uncompressed.data (),
									   uncompressed.size (),
									   /*last=*/true,
...
TEST (SSLClientTest, UpdateCAStoreWithPem_Online) {
	...
	EXPECT_TRUE (res->status == StatusCode::OK_200 || res->status == StatusCode::MovedPermanently_301);
}
...
#	if !defined(_WIN32) || defined(CPPHTTPLIB_DISABLE_WINDOWS_AUTOMATIC_ROOT_CERTIFICATES_UPDATE)
	...
	EXPECT_EQ (static_cast<unsigned long> (X509_V_ERR_HOSTNAME_MISMATCH), res.ssl_backend_error ());
#	endif
#  endif
}
...
TEST (SSLClientTest, WindowsCertificateVerification_DefaultEnabled) {
	...
	auto res = cli.Get ("/");
	if (res) {
		EXPECT_NE (StatusCode::InternalServerError_500, res->status);
	}
}
...
#  ifdef CPPHTTPLIB_OPENSSL_SUPPORT
	...
	EXPECT_TRUE (res.ssl_backend_error () == 0x80000002 || res.ssl_backend_error () == 0xA000126);
#  endif
}
...
TEST (SSLClientTest, ErrorReportingWhenInvalid) {
	...
	SSLClient cli ("localhost", 8080, "nonexistent_cert.pem", "nonexistent_key.pem");
...
#ifdef CPPHTTPLIB_SSL_ENABLED
void
ClientCertPresent (const std::string& client_cert_file,
	...
	SSLClient cli (HOST, PORT, client_cert_file, client_private_key_file, client_encrypted_private_key_pass);
	cli.enable_server_certificate_verification (false);
	cli.set_connection_timeout (30);
...
TEST (SSLClientServerTest, ClientEncryptedCertPresent) {
	ClientCertPresent (CLIENT_ENCRYPTED_CERT_FILE, CLIENT_ENCRYPTED_PRIVATE_KEY_FILE, CLIENT_ENCRYPTED_PRIVATE_KEY_PASS);
}

// PEM memory-based constructor tests (works with all TLS backends)
void
PemMemoryClientCertPresent (const std::string& client_cert_file,
							const std::string& client_private_key_file,
							const std::string& client_encrypted_private_key_pass = std::string ()) {
	...
	SSLClient::PemMemory client_pem = {client_cert_pem.c_str (),
...
TEST (SSLClientServerTest, ClientCertMissing) {
	...
	EXPECT_TRUE (res.error () == Error::SSLServerVerification || res.error () == Error::SSLConnection);
...
TEST (SSLClientServerTest, SSLConnectTimeout) {
	...
	public:
		NoListenSSLServer (const char* cert_path,
...
TEST (SSLClientServerTest, TlsVerifyHostname) {
	...
	svr.Get ("/test",
			 [] (const Request&, Response& res) { res.set_content ("ok", "text/plain"); });
	...
	ASSERT_TRUE (verify_callback_called) << "Verify callback should have been called";
...
TEST (SSLClientServerTest, ClientCAListLoadErrorRecorded) {
	...
	// When SSL_load_client_CA_file fails, last_ssl_error_ should be set
	{
		...
		if (!svr.is_valid ()) {
			EXPECT_NE (0, svr.ssl_last_error ());
		}
	}
}
...
TEST (TlsVerifyErrorTest, GetVerifyErrorString) {
	...
	std::string error_str = TlsError::verify_error_to_string (10);	// X509_V_ERR_CERT_HAS_EXPIRED
...
TEST (DecodeWithChunkedEncoding, BrotliEncoding_Online) {
	...
	auto res = cli.Get ("/ajax/libs/jquery/3.5.1/jquery.js", {{"Accept-Encoding", "br"}});
...
TEST (HttpToHttpsRedirectTest, CertFile) {
	...
	svr.Get ("/index", [&] (const Request&, Response& res) {
		res.set_redirect ("https://127.0.0.1:" + std::to_string (ssl_port) + "/index");
		svr.stop ();
	});
	...
	thread t = thread ([&] () { ASSERT_TRUE (svr.listen ("127.0.0.1", PORT)); });
	thread t2 = thread ([&] () { ASSERT_TRUE (ssl_svr.listen ("127.0.0.1", ssl_port)); });
	auto se = detail::scope_exit ([&] {
		t2.join ();
		t.join ();
		ASSERT_FALSE (svr.is_running ());
...
TEST (SSLClientRedirectTest, CertFile) {
	...
	thread t2 = thread ([&] () { ASSERT_TRUE (ssl_svr2.listen ("127.0.0.1", ssl_port)); });
	auto se = detail::scope_exit ([&] {
		t2.join ();
		t.join ();
...
TEST (MultipartFormDataTest, DataProviderItems) {
	...
	svr.Post ("/post-none", [&] (const Request& req, Response& /*res*/, const ContentReader& content_reader) {
	...
	svr.Post ("/post-items", [&] (const Request& req, Response& /*res*/, const ContentReader& content_reader) {
	...
	svr.Post ("/post-both", [&] (const Request& req, Response& /*res*/, const ContentReader& content_reader) {
		ASSERT_TRUE (req.is_multipart_form_data ());
		std::vector<FormData> items;
		content_reader (
			[&] (const FormData& file) {
...

	{
		...
		{
			auto res = cli.Post ("/post-items", {}, items, providers);	// empty providers
			ASSERT_TRUE (res);
			ASSERT_EQ (StatusCode::OK_200, res->status);
		}
...
TEST (MultipartFormDataTest, BadHeader) {
	...
	const std::string body = "This is the preamble.  It is to be ignored, though it\r\n"
	...
	std::string content_type = R"(multipart/form-data; boundary="simple boundary")";
...
TEST (MultipartFormDataTest, PutFormData) {
	...
	svr.Put ("/put", [&] (const Request& req, const Response& /*res*/, const ContentReader& content_reader) {
		if (req.is_multipart_form_data ()) {
			std::vector<FormData> items;
			content_reader (
				[&] (const FormData& file) {
...
TEST (MultipartFormDataTest, PutFormDataCustomBoundary) {
	...
	svr.Put ("/put_customboundary",
			 [&] (const Request& req, const Response& /*res*/, const ContentReader& content_reader) {
				 if (req.is_multipart_form_data ()) {
					 std::vector<FormData> items;
					 content_reader (
						 [&] (const FormData& file) {
...
TEST (MultipartFormDataTest, LargeHeader) {
	...
	size_t header_dummy_size = read_buff_size - (header_prefix.size () +
												 header_suffix.size () + boundary.size () / 2);
...
TEST (MultipartFormDataTest, ContentProviderCoalescesWrites) {
	...
	for (size_t i = 0; i < kItemCount; i++) {
		items.push_back ({"field" + std::to_string (i), "value" + std::to_string (i), "", ""});
	}
...
TEST (MultipartFormDataTest, MakeFileProvider) {
	...
	Server svr;
	svr.Post ("/upload", [&] (const Request& req, Response& /*res*/, const ContentReader& content_reader) {
		ASSERT_TRUE (req.is_multipart_form_data ());
		std::vector<FormData> items;
		content_reader (
			[&] (const FormData& file) {
	...
	FormDataProviderItems providers;
	providers.push_back (make_file_provider ("myfile", tmp_path, "data.bin", "application/octet-stream"));
...
TEST (MakeFileBodyTest, Basic) {
	...
	Client cli (HOST, port);
	auto res = cli.Post ("/upload", fb.first, fb.second, "application/octet-stream");
	ASSERT_TRUE (res);
	EXPECT_EQ (StatusCode::OK_200, res->status);
}
...
TEST (TaskQueueTest, IncreaseAtomicIntegerWithQueueLimit) {
	...
	std::atomic_uint count{0};
	std::unique_ptr<TaskQueue> task_queue{new ThreadPool{/*num_threads=*/1, /*max_threads=*/1, qlimit}};
...
TEST (RedirectTest, RedirectWithPlusInPath) {
	...
	svr.Get ("/", [] (const Request& /*req*/, Response& res) { res.set_redirect ("/a+b"); });
		...
		while ((n = ::recv (cli, buf, sizeof (buf), 0)) > 0) {
			...
			while ((pos = buf_all.find ("\r\n\r\n")) != std::string::npos) {
				...
				if (e != std::string::npos) {
					...
					std::string msg = "CRLF injection detected in request line: '" + request_line + "'";
...
TEST (PathParamsTest, MultipleParams) {
	...
	std::unordered_map<std::string, std::string> expected_params = {{"userid", "42"}, {"subid", "2"}};
...
TEST (PathParamsTest, SequenceOfParams) {
	...
	std::unordered_map<std::string, std::string> expected_params = {{"x", "1"}, {"y", "2"}, {"z", "3"}};
...
TEST (UniversalClientImplTest, Ipv6LiteralAddress) {
	...
	Client cli (ipV6TestURL + ":" + std::to_string (port), CLIENT_CERT_FILE, CLIENT_PRIVATE_KEY_FILE);
...
TEST (MakeHostAndPortStringTest, VariousPatterns) {
	...
	EXPECT_EQ ("example.com", detail::make_host_and_port_string ("example.com", 80, false));

	// IPv4 with default HTTPS port (443)
	EXPECT_EQ ("example.com", detail::make_host_and_port_string ("example.com", 443, true));

	// IPv4 with non-default HTTP port
	EXPECT_EQ ("example.com:8080", detail::make_host_and_port_string ("example.com", 8080, false));

	// IPv4 with non-default HTTPS port
	EXPECT_EQ ("example.com:8443", detail::make_host_and_port_string ("example.com", 8443, true));
	...
	EXPECT_EQ ("[::1]:8080", detail::make_host_and_port_string ("::1", 8080, false));
	...
	EXPECT_EQ ("[::1]:3000", detail::make_host_and_port_string ("::1", 3000, false));

	// IPv6 with zone ID (link-local address) with default port
	EXPECT_EQ ("[fe80::1%eth0]", detail::make_host_and_port_string ("fe80::1%eth0", 80, false));

	// IPv6 with zone ID (link-local address) with non-default port
	EXPECT_EQ ("[fe80::1%eth0]:8080", detail::make_host_and_port_string ("fe80::1%eth0", 8080, false));

	// Edge case: Port 443 with is_ssl=false (should add port)
	EXPECT_EQ ("example.com:443", detail::make_host_and_port_string ("example.com", 443, false));

	// Edge case: Port 80 with is_ssl=true (should add port)
	EXPECT_EQ ("example.com:80", detail::make_host_and_port_string ("example.com", 80, true));
	...
	EXPECT_EQ ("[::1]", detail::make_host_and_port_string ("[::1]", 443, true));
	EXPECT_EQ ("[::1]:8080", detail::make_host_and_port_string ("[::1]", 8080, false));
	EXPECT_EQ ("[2001:db8::1]:8080", detail::make_host_and_port_string ("[2001:db8::1]", 8080, false));
	EXPECT_EQ ("[fe80::1%eth0]", detail::make_host_and_port_string ("[fe80::1%eth0]", 80, false));
	...
	EXPECT_EQ ("[host:name]", detail::make_host_and_port_string ("host:name", 80, false));

	// Port number edge cases (no validation, but should not crash)
	EXPECT_EQ ("example.com:0", detail::make_host_and_port_string ("example.com", 0, false));
	EXPECT_EQ ("example.com:-1", detail::make_host_and_port_string ("example.com", -1, false));
	EXPECT_EQ ("example.com:65535", detail::make_host_and_port_string ("example.com", 65535, false));
	EXPECT_EQ ("example.com:65536", detail::make_host_and_port_string ("example.com", 65536, false));
}
...
TEST (DirtyDataRequestTest, HeadFieldValueContains_CR_LF_NUL) {
	...
	svr.Get ("/test",
			 [&] (const Request& /*req*/, Response& res) { EXPECT_EQ (res.status, 400); });
...
TEST (InvalidHeaderCharsTest, OnServer) {
	...
	svr.Get ("/test_name", [&] (const Request& req, Response& res) {
		...
		if (req.has_param ("header")) {
			header = req.get_param_value ("header");
		}
	...
	Client cli (HOST, PORT);
	{
		auto res = cli.Get (R"(/test_name?header=Value%00%0d%0aHEADER_KEY%3aHEADER_VALUE%0d%0a%0d%0aBODY_BODY_BODY)");
	...
	{
		auto res = cli.Get (R"(/test_value?header=Value%00%0d%0aHEADER_KEY%3aHEADER_VALUE%0d%0a%0d%0aBODY_BODY_BODY)");
...
TEST (InvalidHeaderValueTest, InvalidContentLength) {
	...
	std::string response;
	ASSERT_TRUE (send_request (1, req, &response));
	ASSERT_EQ ("HTTP/1.1 400 Bad Request", response.substr (0, response.find ("\r\n")));
}
...
TEST (Expect100ContinueTest, ServerClosesConnection) {
	...
	{
		const auto curl = std::unique_ptr<CURL, decltype (&curl_easy_cleanup)>{curl_easy_init (),
																			   &curl_easy_cleanup};
		...
		auto list = std::unique_ptr<curl_slist, decltype (&curl_slist_free_all)>{
			curl_slist_append (nullptr, "Content-Type: application/octet-stream"), &curl_slist_free_all};
		...
		using read_callback_t = size_t (*) (char* ptr, size_t size, size_t nmemb, void* userdata);
		read_callback_t read_callback = [] (char* ptr, size_t size, size_t nmemb, void* userdata) -> size_t {
			read_data* d = (read_data*) userdata;

			if (!userdata || d->read_size >= d->total_size) {
				return 0;
			}
		...
		write_callback_t write_callback = [] (char* ptr, size_t size, size_t nmemb, void* userdata) -> size_t {
...

		{
			...
			const auto res = curl_easy_getinfo (curl.get (), CURLINFO_RESPONSE_CODE, &response_code);
			ASSERT_EQ (res, CURLE_OK);
...
inline void
max_timeout_test (S& svr, C& cli, time_t timeout, time_t threshold) {
...
TEST (ClientInThreadTest, Issue2068) {
...

	{
		...
		std::thread t ([&] {
			client->Get ("/event1", [&] (const char*, size_t) -> bool { return !stop; });
		});
...
TEST (ForwardedHeadersTest, MultipleTrustedProxies_UsesClientIP) {
	...
	Client cli (HOST, PORT);
	auto res = cli.Get ("/ip", {{"X-Forwarded-For", "198.51.100.23, 203.0.113.66, 192.0.2.45"}});
...
TEST (ForwardedHeadersTest, TrustedProxyNotInHeader_UsesFirstFromXFF) {
	...
	Client cli (HOST, PORT);
	auto res = cli.Get ("/ip", {{"X-Forwarded-For", "198.51.100.23, 198.51.100.24"}});
...
static void
run_malformed_xff_test (const std::string& xff_value) {
	Server svr;
	svr.set_trusted_proxies ({"192.0.2.45"});
...
inline std::string
read_all (ClientImpl::StreamHandle& handle) {
	std::string result;
	char buf [8192];
	ssize_t n;
...
public:
	...
	read (char* ptr, size_t size) override {
		...
		size_t limit = error_after >= 0 ? static_cast<size_t> (error_after) : data.size ();
	...
	ssize_t
	write (const char*, size_t) override {
		return -1;
	}
	void
...
protected:
	...
	SetUp () override {
		...
		svr_.Get ("/chunked", [] (const Request&, Response& res) {
			res.set_chunked_content_provider ("text/plain", [] (size_t offset, DataSink& sink) {
				if (offset < 15) {
					sink.write ("chunk", 5);
					return true;
				}
		...
		svr_.Get ("/streamed-chunked-with-prohibited-trailer", [] (const Request& /*req*/, Response& res) {
			auto i = new int (0);
			res.set_header ("Trailer", "Content-Length, X-Allowed");
			res.set_chunked_content_provider (
				"text/plain",
				[i] (size_t /*offset*/, DataSink& sink) {
					switch (*i) {
						...
						case 2: sink.os << "789"; break;
						case 3: {
							sink.done_with_trailer ({{"Content-Length", "5"}, {"X-Allowed", "yes"}});
						} break;
					}
					(*i)++;
					return true;
...
TEST_F (OpenStreamTest, DefaultHeaders) {
	...
	// open_stream GET should include Host, User-Agent and Accept-Encoding
	{
		...
		EXPECT_NE (body.find ("Host:127.0.0.1:" + std::to_string (port_)), std::string::npos);
		EXPECT_NE (body.find ("User-Agent:cpp-httplib/" CPPHTTPLIB_VERSION), std::string::npos);
	...
	{
		auto handle = cli.open_stream (
			"POST", "/echo-headers", {}, {{"Content-Type", "application/custom"}}, "{}", "application/custom");
		ASSERT_TRUE (handle.is_valid ());
		auto body = read_all (handle);
	...
	{
		auto handle = cli.open_stream ("GET", "/echo-headers", {}, {{"User-Agent", "MyAgent/1.2"}});
		ASSERT_TRUE (handle.is_valid ());
		auto body = read_all (handle);
...
TEST_F (OpenStreamTest, Chunked) {
	...
	EXPECT_TRUE (handle.response &&
				 handle.response->get_header_value ("Transfer-Encoding") == "chunked");
	EXPECT_EQ ("chunkchunkchunk", read_all (handle));
}

TEST_F (OpenStreamTest, ProhibitedTrailersAreIgnored_Stream) {
	Client cli ("127.0.0.1", port_);
	auto handle = cli.open_stream ("GET", "/streamed-chunked-with-prohibited-trailer");
	ASSERT_TRUE (handle.is_valid ());
	...
	EXPECT_EQ (std::string ("yes"), handle.response->get_trailer_value ("X-Allowed"));

	// Verify trailers are NOT present as regular headers
	EXPECT_EQ (std::string (""), handle.response->get_header_value ("Content-Length"));
	EXPECT_EQ (std::string (""), handle.response->get_header_value ("X-Allowed"));
}

static std::thread
serve_single_response (std::promise<int>& port_promise, const std::string& response) {
...
TEST (OpenStreamMalformedContentLength, InvalidArgument) {
	...
	auto server_thread = serve_single_response (port_promise,
												"HTTP/1.1 200 OK\r\n"
												"Content-Type: text/plain\r\n"
...
TEST_F (OpenStreamTest, Gzip) {
	Client cli ("127.0.0.1", port_);
	auto handle = cli.open_stream ("GET", "/compressible", {}, {{"Accept-Encoding", "gzip"}});
...
TEST_F (OpenStreamTest, Brotli) {
	Client cli ("127.0.0.1", port_);
	auto handle = cli.open_stream ("GET", "/compressible", {}, {{"Accept-Encoding", "br"}});
...
TEST_F (OpenStreamTest, Zstd) {
	Client cli ("127.0.0.1", port_);
	auto handle = cli.open_stream ("GET", "/compressible", {}, {{"Accept-Encoding", "zstd"}});
...
protected:
	...
	SetUp () override {
		...
		svr_.Post ("/chunked-response", [] (const Request& req, Response& res) {
			...
			res.set_chunked_content_provider ("text/plain", [body] (size_t offset, DataSink& sink) {
				if (offset < body.size ()) {
...
TEST_F (SSLOpenStreamTest, Post) {
	...
	auto handle = cli.open_stream ("POST", "/echo", {}, {}, "Hello SSL POST", "text/plain");
...
TEST_F (SSLOpenStreamTest, PostChunked) {
	...
	auto handle = cli.open_stream ("POST", "/chunked-response", {}, {}, "Chunked SSL Data", "text/plain");
	...
	svr.Get ("/no-content-length", [&] (const Request& /*req*/, Response& res) {
		res.set_content_provider ("text/plain", [&] (size_t offset, DataSink& sink) -> bool {
			if (offset < expected_body.size ()) {
				sink.write (expected_body.data () + offset, expected_body.size () - offset);
			}
			sink.done ();
			return true;
		});
...
static std::string
compress_payload_for_parity (const std::string& in) {
	std::string out;
	Compressor compressor;
	bool ok = compressor.compress (in.data (), in.size (), /*last=*/true, [&] (const char* data, size_t n) {
		out.append (data, n);
		return true;
	});
	EXPECT_TRUE (ok);
...
template <typename Compressor>
static void
test_compression_parity (const std::string& original, const std::string& path, const std::string& encoding) {
	const std::string compressed = compress_payload_for_parity<Compressor> (original);
...
TEST (ParityTest, Zstd) {
	test_compression_parity<detail::zstd_compressor> ("Zstandard parity test payload", "/parity-zstd", "zstd");
}
#endif
...
inline std::string
read_body (httplib::stream::Result& result) {
	std::string body;
	while (result.next ()) {
...
protected:
	...
	SetUp () override {
		...
		svr_.Get ("/echo-params", [] (const httplib::Request& req, httplib::Response& res) {
			std::string r;
		...
		svr_.Post ("/echo-headers", [] (const httplib::Request& req, httplib::Response& res) {
			std::string r;
			for (const auto& h: req.headers) r += h.first + ": " + h.second + "\n";
			res.set_content (r, "text/plain");
		});
		svr_.Post ("/echo-params", [] (const httplib::Request& req, httplib::Response& res) {
			std::string r = "params:";
			for (const auto& p: req.params) r += p.first + "=" + p.second + ";";
		...
		svr_.Patch ("/echo", [] (const httplib::Request& req, httplib::Response& res) {
			res.set_content ("PATCH:" + req.body, "text/plain");
		});
		svr_.Delete ("/resource", [] (const httplib::Request& req, httplib::Response& res) {
			res.set_content (req.body.empty () ? "Deleted" : "Deleted:" + req.body, "text/plain");
		});
		svr_.Get ("/head-test", [] (const httplib::Request&, httplib::Response& res) {
			res.set_content ("body for HEAD", "text/plain");
		});
		svr_.Options ("/options", [] (const httplib::Request&, httplib::Response& res) {
...
TEST_F (StreamApiTest, PostBasic) {
	httplib::Client cli (HOST, PORT);
	auto result = httplib::stream::Post (cli, "/echo", R"({"key":"value"})", "application/json");
	ASSERT_TRUE (result.is_valid ());
...
TEST_F (StreamApiTest, PostWithHeaders) {
	...
	auto result = httplib::stream::Post (cli, "/echo-headers", headers, "body", "text/plain");
...
TEST_F (StreamApiTest, PostWithParams) {
	...
	auto result = httplib::stream::Post (cli, "/echo-params", params, "data", "text/plain");
...
TEST (ErrorHandlingTest, StreamReadTimeout) {
	...
	svr.Get ("/slow-stream", [] (const Request&, Response& res) {
		...
		res.set_content_provider (1000,	 // content_length
								  "text/plain",
								  [] (size_t offset, size_t /*length*/, DataSink& sink) {
									  if (offset < 100) {
	...
	if (n < 0) {
		...
		EXPECT_TRUE (handle.get_read_error () == Error::Timeout || handle.get_read_error () == Error::Read)
			<< "Actual error: " << to_string (handle.get_read_error ());
	}

	// Either we got an error, or we got less data than expected
	EXPECT_TRUE (got_error || total < 1000) << "Expected timeout but got all " << total << " bytes";
...
TEST (ErrorHandlingTest, StreamConnectionClosed) {
	...
	svr.Get ("/will-close", [&] (const Request&, Response& res) {
		res.set_content_provider (10000,  // Large content_length that we won't fully send
								  "text/plain",
								  [&] (size_t offset, size_t /*length*/, DataSink& sink) {
									  if (offset < 100) {
	...
	if (n < 0) {
		EXPECT_TRUE (handle.get_read_error () == Error::ConnectionClosed || handle.get_read_error () == Error::Read)
			<< "Actual error: " << to_string (handle.get_read_error ());
	}
...
TEST (ErrorHandlingTest, SSLStreamReadTimeout) {
	...
	svr.Get ("/slow-stream", [] (const Request&, Response& res) {
		res.set_content_provider (1000, "text/plain", [] (size_t offset, size_t /*length*/, DataSink& sink) {
			if (offset < 100) {
				std::string data (100, 'A');
				sink.write (data.c_str (), data.size ());
				return true;
...
TEST (ErrorHandlingTest, SSLStreamConnectionClosed) {
	...
	svr.Get ("/will-close", [&] (const Request&, Response& res) {
		res.set_content_provider (10000, "text/plain", [&] (size_t offset, size_t /*length*/, DataSink& sink) {
			if (offset < 100) {
				std::string data (100, 'X');
				sink.write (data.c_str (), data.size ());
				return true;
...
TEST (ETagTest, IfNoneMatchBoundaryCheck) {
	...
	Headers h4 = {{"If-None-Match",
				   "W/\"first-long-non-matching-value\", "
...
TEST (ETagTest, LastModifiedAndIfModifiedSince) {
	...
	ASSERT_TRUE (res1->has_header ("ETag"));
	std::string etag = res1->get_header_value ("ETag");
	Headers h5 = {{"If-None-Match", etag}, {"If-Modified-Since", "Sun, 01 Jan 2000 00:00:00 GMT"}};
...
TEST (ETagTest, IfRangeWithDate) {
	...
	Headers h3 = {{"Range", "bytes=5-9"}, {"If-Range", "Sun, 01 Jan 2000 00:00:00 GMT"}};
	...
	Headers h4 = {{"Range", "bytes=0-4"}, {"If-Range", "Sun, 01 Jan 2099 00:00:00 GMT"}};
...
TEST (ETagTest, InvalidIfModifiedSinceAndIfRangeDate) {
	...
	// content (200)
	Headers h_ifrange_bad = {{"Range", "bytes=0-3"}, {"If-Range", "invalid-date"}};
...
TEST (ETagTest, ExtremeLargeDateValues) {
	...
	Headers h_large_date = {{"If-Modified-Since", "Sun, 01 Jan 99999 00:00:00 GMT"}};
	...
	// content (200)
	Headers h_ifrange_large = {{"Range", "bytes=0-3"}, {"If-Range", "Sun, 01 Jan 99999 00:00:00 GMT"}};
...
TEST (ETagTest, NegativeFileModificationTime) {
	...
	if (set_negative) {
		...
		Headers h_bad_date = {{"If-Modified-Since", "Sun, 01 Jan 99999 00:00:00 GMT"}};
...
protected:
	...
	static bool
	parse_sse_line (const std::string& line, sse::SSEMessage& msg, int& retry_ms) {
...
protected:
	...
	setup_server () {
		...
		server_->Get ("/events", [this] (const Request& req, Response& res) {
			...
			if (!last_id.empty ()) {
				last_received_event_id_ = last_id;
			}

			res.set_chunked_content_provider ("text/event-stream", [this] (size_t /*offset*/, DataSink& sink) {
				std::unique_lock<std::mutex> lock (event_mutex_);
				if (event_cv_.wait_for (lock, std::chrono::milliseconds (200), [this] {
	...
	void
	send_event (const std::string& event) {
...
TEST_F (SSEIntegrationTest, ManualStop) {
	...
	server_->Get ("/stay-open", [&handler_running] (const Request&, Response& res) {
		res.set_chunked_content_provider ("text/event-stream", [&handler_running] (size_t offset, DataSink& sink) {
			if (offset == 0) {
...
TEST_F (SSEIntegrationTest, AutoReconnectAfterDisconnect) {
	...
	server_->Get ("/reconnect-test", [&connection_count] (const Request&, Response& res) {
	...
	sse.on_message (
		[&message_count] (const sse::SSEMessage&) { message_count.fetch_add (1); });
...
TEST_F (SSEIntegrationTest, LastEventIdSentOnReconnect) {
	...
	server_->Get ("/reconnect-with-id", [&] (const Request& req, Response& res) {
		...
		res.set_chunked_content_provider ("text/event-stream", [conn] (size_t offset, DataSink& sink) {
			if (offset == 0) {
				std::string event = "id: event-" + std::to_string (conn) + "\ndata: msg\n\n";
				sink.write (event.data (), event.size ());
			}
			return false;
		});
...
TEST_F (SSEIntegrationTest, SetHeadersUpdatesOnReconnect) {
	...
	sse.on_open ([&sse] () { sse.set_headers ({{"Authorization", "Bearer new-token"}}); });
...
TEST_F (SSEIntegrationTest, ReconnectOn401WithHeaderRefresh) {
	...
	sse.on_error ([&sse] (Error) { sse.set_headers ({{"Authorization", "Bearer valid"}}); });
...
TEST (ZipBombProtectionTest, DecompressedSizeExceedsLimit) {
	...
	svr.Post ("/test", [&] (const Request& req, Response& res) {
		res.set_content ("Body size: " + std::to_string (req.body.size ()), "text/plain");
	});
	...
	compressor.compress (original_data.data (), original_data.size (), true, [&] (const char* data, size_t size) {
	...
	Client cli (HOST, PORT);
	Headers headers = {{"Content-Encoding", "gzip"}};
	auto res = cli.Post ("/test", headers, compressed_data, "application/octet-stream");
...
#ifdef CPPHTTPLIB_OPENSSL_SUPPORT
X509*
readCertificate (const std::string& strFileName) {
...
EVP_PKEY*
readPrivateKey (const std::string& strFileName) {
	std::ifstream inStream (strFileName);
...
TEST (BindServerTest, UpdateCerts) {
	...
	svr.update_certs_pem (SERVER_CERT_FILE, SERVER_PRIVATE_KEY_FILE, CLIENT_CA_CERT_FILE);
...
TEST (SSLClientServerTest, CustomizeServerSSLCtxMbedTLS) {
	...
	auto setup_callback = [&callback_invoked] (void* ctx) {
		...
		if (!initialized) {
			...
			if (mbedtls_pk_parse_keyfile (&own_key,
...
TEST (WebSocketTest, RSVBitsMustBeZero) {
	...
	// RSV1 set (0x40)
	{
		...
		std::string payload;
		bool fin;
		EXPECT_FALSE (ws::impl::read_websocket_frame (strm, opcode, payload, fin, false, 1024));
	}
	...
	// No RSV bits set - should succeed
	{
		...
		std::string payload;
		bool fin;
		EXPECT_TRUE (ws::impl::read_websocket_frame (strm, opcode, payload, fin, false, 1024));
...
TEST (WebSocketTest, InvalidUTF8TextFrame) {
	...
	EXPECT_FALSE (ws::impl::is_valid_utf8 ("\xED\xA0\x80"));  // Surrogate half U+D800
...
TEST (WebSocketTest, UnsupportedScheme) {
	...
	EXPECT_FALSE (ws3.is_valid ());
#else
	EXPECT_THROW (ws::WebSocketClient ("http://localhost:8080/path"), std::invalid_argument);

	EXPECT_THROW (ws::WebSocketClient ("ftp://localhost:8080/path"), std::invalid_argument);
#endif
}
...
protected:
	...
	setup_server () {
		...
		server_->WebSocket ("/ws-echo-string", [] (const Request&, ws::WebSocket& ws) {
			std::string msg;
			while (ws.read (msg)) {
				ws.send ("echo: " + msg);
			}
		...
		server_->WebSocket ("/ws-request-info", [] (const Request& req, ws::WebSocket& ws) {
		...
		server_->WebSocket ("/ws-close-status", [] (const Request&, ws::WebSocket& ws) {
			std::string msg;
			ws.read (msg);	// wait for a message
...
TEST_F (WebSocketIntegrationTest, TextEcho) {
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-echo");
	ASSERT_TRUE (client.connect ());
...
TEST_F (WebSocketIntegrationTest, CloseHandshake) {
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-close");
	ASSERT_TRUE (client.connect ());
...
TEST_F (WebSocketIntegrationTest, ReadString) {
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-echo-string");
	ASSERT_TRUE (client.connect ());
...
TEST_F (WebSocketIntegrationTest, RequestAccess) {
	...
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-request-info", headers);
	ASSERT_TRUE (client.connect ());
...
TEST_F (WebSocketIntegrationTest, ConnectToInvalidPath) {
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/nonexistent");
...
TEST_F (WebSocketIntegrationTest, CloseWithStatus) {
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-close-status");
	ASSERT_TRUE (client.connect ());
...
TEST_F (WebSocketIntegrationTest, SubProtocolNegotiation) {
	...
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-subprotocol", headers);
	ASSERT_TRUE (client.connect ());
...
TEST_F (WebSocketIntegrationTest, SubProtocolNotRequested) {
	...
	ws::WebSocketClient client ("ws://localhost:" + std::to_string (port_) + "/ws-subprotocol");
	ASSERT_TRUE (client.connect ());
...
TEST (WebSocketPreRoutingTest, RejectWithoutAuth) {
	...
	ws::WebSocketClient client2 ("ws://localhost:" + std::to_string (port) + "/ws", headers);
...
protected:
	void
	SetUp () override {
		server_ = httplib::detail::make_unique<SSLServer> (SERVER_CERT_FILE, SERVER_PRIVATE_KEY_FILE);
		server_->WebSocket ("/ws-echo", [] (const Request&, ws::WebSocket& ws) {
			std::string msg;
			ws::ReadResult ret;
			while ((ret = ws.read (msg))) {
...
TEST_F (WebSocketSSLIntegrationTest, TextEcho) {
	ws::WebSocketClient client ("wss://localhost:" + std::to_string (port_) + "/ws-echo");
...
TEST (RequestSmugglingTest, UnconsumedGETBodyOnFileHandler) {
	...
	auto error = Error::Success;
	auto sock = detail::create_client_socket ("localhost",
											  "",
											  port,
											  AF_UNSPEC,
											  false,
	...
	for (;;) {
		...
		if (hdr_end != std::string::npos) {
			...
			if (cl_pos != std::string::npos) {
				...
				auto cl = std::stoul (first_response.substr (cl_val_start, cl_val_end - cl_val_start));
				if (first_response.size () >= hdr_end + 4 + cl) {
					break;
				}
			}
			else {
...
TEST (RequestSmugglingTest, ContentLengthAndTransferEncodingRejected) {
	...
	// Exact "chunked"
	{
		...
		std::string response;
		ASSERT_TRUE (send_request (1, req, &response));
		EXPECT_EQ ("HTTP/1.1 400 Bad Request", response.substr (0, response.find ("\r\n")));
	}
...
TEST (KeepAliveTest, DeleteWithoutContentLengthDoesNotEatNextRequest) {
	...
	auto error = Error::Success;
	auto sock = detail::create_client_socket (HOST,
											  "",
											  port,
											  AF_UNSPEC,
											  false,
	...
	auto send_request_and_read_response = [&] (const std::string& req, std::string& out) -> bool {
