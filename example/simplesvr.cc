...
string
dump_headers (const Headers& headers) {
	string s;
	char buf [BUFSIZ];
...
string
dump_multipart_formdata (const MultipartFormData& form) {
	string s;
	char buf [BUFSIZ];
...
string
log (const Request& req, const Response& res) {
	string s;
	char buf [BUFSIZ];
	...
	snprintf (
		buf, sizeof (buf), "%s %s %s", req.method.c_str (), req.version.c_str (), req.path.c_str ());
	s += buf;
